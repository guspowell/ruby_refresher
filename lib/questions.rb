...
def array_sort_by_last_letter_of_word(array)
  array.sort {|left,right| left[-1] <=> right[-1] }
end
...
def get_first_half_of_string(string)
  string.length % 2 == 0 ? string[0..string.length/2-1] : string[0..string.length/2]
end
...
def total_of_array(array)
  array.inject { |memo,index| memo + index }
end
...
def add_together_keys_and_values(hash)
  hash.inject(0) { |result, (k,v)| result + k + v }
end
...
def remove_capital_letters_from_string(string)
  array = string.chars
  array.delete_if { |char| char == char.upcase unless char == " " }
  array.join('')
end
...
def get_domain_name_from_email_address(email)
  domain_with_end = string.split('@')[1]
  domain = domain_with_end[/[^.]+/]
  domain
end
